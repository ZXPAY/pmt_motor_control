 ...
 * @file LPC1769.h
 * @author LiYu87
 * @brief LPC1769 Register Definitions and Cortex-M3 Supplement Definitions
 * @date 2020.02.17
 */
...
void __set_scs_reg(volatile uint32_t *reg,
                   uint32_t val); /* Write a register in SCS */
#define __enable_irqn_user(n) __set_scs_reg(&ISER[((n) / 32)], 1 << ((n) % 32))
#define __disable_irqn_user(n) __set_scs_reg(&ISCR[((n) / 32)], 1 << ((n) % 32))
#define __test_irqn_enabled_user(n)                                            \
  (__get_scs_reg(&ISCR[(n) / 32]) & (1 << ((n) % 32)))
...
#define __test_active_irqn_user(n)                                             \
  (__get_scs_reg(&IABR[(n) / 32]) & (1 << ((n) % 32)))
...
#define __STREXB(d, p)                                                         \
  ({                                                                           \
    register uint32_t __r asm("r2");                                           \
    register uint8_t __d asm("r1") = d;                                        \
    register volatile uint8_t *__p asm("r0") = p;                              \
    asm("STREXB %0,%2,[%1]\n" : "=r"(__r) : "r"(__p), "r"(__d));               \
    __r;                                                                       \
  })
#define __STREXH(d, p)                                                         \
  ({                                                                           \
    register uint32_t __r asm("r2");                                           \
    register uint16_t __d asm("r1") = d;                                       \
    register volatile uint16_t *__p asm("r0") = p;                             \
    asm("STREXH %0,%2,[%1]\n" : "=r"(__r) : "r"(p), "r"(__d));                 \
    __r;                                                                       \
  })
#define __STREXW(d, p)                                                         \
  ({                                                                           \
    register uint32_t __r asm("r2");                                           \
    register uint32_t __d asm("r1") = d;                                       \
    register volatile uint32_t *__p asm("r0") = p;                             \
    asm("STREX  %0,%2,[%1]\n" : "=r"(__r) : "r"(p), "r"(__d));                 \
    __r;                                                                       \
  })
...
#define __set_PCLKSEL(p, v)                                                    \
  PCLKSEL[(p) / 16] =                                                          \
      (PCLKSEL[(p) / 16] & ~(3 << ((p)*2 % 32))) | (v << ((p)*2 % 32))
#define PCLKDIV_4 0
#define PCLKDIV_1 1
#define PCLKDIV_2 2
#define PCLKDIV_8 3
...
#define __set_PINSEL(p, b, v)                                                  \
  PINSEL[(p)*2 + (b) / 16] =                                                   \
      (PINSEL[(p)*2 + (b) / 16] & ~(3 << ((b)*2 % 32))) | (v << ((b)*2 % 32))
#define __set_PINMODE(p, b, v)                                                 \
  PINMODE[(p)*2 + (b) / 16] =                                                  \
      (PINMODE[(p)*2 + (b) / 16] & ~(3 << ((b)*2 % 32))) | (v << ((b)*2 % 32))
